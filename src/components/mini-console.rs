        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            *move_listener.lock().expect("move listener set lock") = Some(move_handle);
            ...
            let up_handle = window_event_listener(ev::mouseup, move |_ev: web_sys::MouseEvent| {
                set_resize_state.set(None);
                if let Some(handle) = move_listener_for_up.lock().expect("move listener up lock").take() {
                    handle.remove();
                }
                if let Some(handle) = up_listener_for_up.lock().expect("up listener up lock").take() {
                    handle.remove();
                }
            });
